...
namespace hdt::havok
{
    inline auto getAnimationSkeleton(const RE::Actor* a_actor) -> const RE::hkaSkeleton*
    {
        ...
        return graph->characterInstance.setup->animationSkeleton.get();
    }

    inline auto getReferencePoseByIndex(const RE::hkaSkeleton* a_skeleton, const std::int32_t a_boneIndex,
                                        RE::hkQsTransform& a_outTransform) -> bool
    {
    ...
    inline auto hKQsTransformToNiTransform(const RE::hkQsTransform& a_hkTransform) -> RE::NiTransform
    {
        ...
        auto cross = [&](const float a, const float b, const float c, const float d, const int i, const int j)
        {
