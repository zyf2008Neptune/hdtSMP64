...
namespace hdt
{
    ...
    inline auto isVRNiStreamStub(const RE::NiAVObject* obj) -> bool
    {
        ...
        const auto vtbl = *reinterpret_cast<const uintptr_t* const*>(obj);
        const auto slotGetObjectByName = vtbl[0x158 / sizeof(uintptr_t)]; // 0x158 = slot 0x2B (VR)
        return slotGetObjectByName == 0 || slotGetObjectByName > kCanonicalUserSpaceMax;
    }

    inline auto setNiNodeName(RE::NiNode* node, const char* name) -> void { node->name = name; }
    ...
    inline auto readAllFile(const char* path) -> std::string
    {
        ...
        //
        const size_t required = stream.stream->totalSize;
    ...
    inline auto rotate(const RE::NiPoint3& v, const RE::NiPoint3& axis, const float theta) -> RE::NiPoint3
    {
