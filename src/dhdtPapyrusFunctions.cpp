...
static bool RegisterFuncs(RE::BSScript::IVirtualMachine* registry)
{
	//
...
bool hdt::papyrus::ReloadPhysicsFile(RE::StaticFunctionTag*, RE::Actor* on_actor, RE::TESObjectARMA* on_item,
...
bool hdt::papyrus::SwapPhysicsFile(RE::StaticFunctionTag*, RE::Actor* on_actor, RE::BSFixedString old_physics_file_path,
...
RE::BSFixedString hdt::papyrus::QueryCurrentPhysicsFile(RE::StaticFunctionTag*, RE::Actor* on_actor,
...
bool hdt::papyrus::impl::ReloadPhysicsFileImpl(uint32_t on_actor_formID, uint32_t on_item_formID,
	std::string physics_file_path, bool persist, bool verbose_log)
{
				...
				if (armorName.find(buffer) != std::string::npos) {
					...
					hdt::SkyrimPhysicsWorld::get()->suspendSimulationUntilFinished([&]() {
						system = SkyrimSystemCreator().createOrUpdateSystem(skeleton.npc.get(), armor.armorWorn.get(),
							&armor.physicsFile, std::move(armor.renameMap), armor.m_physics.get());
...
bool hdt::papyrus::impl::SwapPhysicsFileImpl(uint32_t on_actor_formID, std::string old_physics_file_path,
	std::string new_physics_file_path, bool persist, bool verbose_log)
{
	...
	if (verbose_log) {
		RE::ConsoleLog::GetSingleton()->Print("[DynamicHDT] -- Character (%08X) {}, Physics file path {}.",
			on_actor_formID, character_found ? "found" : "not found", armor_addon_found ? "found" : "not found");
	}
...
std::string hdt::papyrus::impl::QueryCurrentPhysicsFileImpl(uint32_t on_actor_formID, uint32_t on_item_formID,
	bool verbose_log)
{
	const auto& AM = hdt::ActorManager::instance();
