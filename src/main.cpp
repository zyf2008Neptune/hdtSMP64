...
namespace
{
    auto ParsePositiveDecimal(const std::string_view a_value, const std::uint64_t a_fallback) -> std::uint64_t
    {
        if (a_value.empty())
        {
            return a_fallback;
...
namespace
{
    auto GetTextureFromIndex(RE::BSLightingShaderMaterial* material, const std::uint32_t index)
        -> RE::NiSourceTexturePtr*
    {
        ...
        case RE::BSTextureSet::Texture::kHeight:
        {
            ...
            {
                return std::addressof(static_cast<RE::BSLightingShaderMaterialEye*>(
                                          static_cast<RE::BSLightingShaderMaterialBase*>(material))
                                          ->envTexture);
            }
            ...
            if (!children.empty())
            {
                for (uint16_t i = 0; i < children.size(); i++)
                {
                    if (RE::NiPointer<RE::NiAVObject> object = children[i])
                    {
                        ...
                        {
                            logger::info("{} {} [{:.2f}, {:.2f}, {:.2f}] - Geometry", object->GetRTTI()->name,
                                         object->name, geometry->world.translate.x, geometry->world.translate.y,
                        ...
                        else
                        {
                            logger::info("{} {} [{:.2f}, {:.2f}, {:.2f}]", object->GetRTTI()->name, object->name,
                                         object->world.translate.x, object->world.translate.y,
                ...
                for (const auto& armor : skeleton.getArmors())
                {
                    RE::ConsoleLog::GetSingleton()->Print(
                        ...
                        armor.state() != hdt::ActorManager::ItemState::e_NoPhysics
                            ? armor.state() == hdt::ActorManager::ItemState::e_Active ? "has active physics system"
...
namespace
{
        ...
        -> bool
    {
        ...
        if (!RE::Script::ParseParameters(a_paramInfo, a_scriptData, a_opcodeOffsetPtr, a_thisObj, a_containingObj,
                                         a_scriptObj, a_locals, buffer, buffer2, buffer3))
        {
            return false;
        }
        ...
        {
            static auto profilerCaptureRequested = false;
            ...
            if (profilerCaptureRequested)
            {
                RE::ConsoleLog::GetSingleton()->Print(
                    ...
                    static_cast<uint64_t>(sampleFrames), static_cast<uint64_t>(printFrames));
