...
namespace hdt
{
	static inline auto convertFloat(const std::string& str) -> float
	{
	...
	static inline auto convertInt(const std::string& str) -> int
	{
	...
	static inline auto convertBool(const std::string& str) -> bool
	{
		if (str == "true" || str == "1") {
			return true;
		}
	...
	auto XMLReader::GetInspected() -> Xml::Inspected
	{
	...
	auto XMLReader::hasAttribute(const std::string& a_name) -> bool
	{
	...
	auto XMLReader::getAttribute(const std::string& a_name) -> std::string
	{
		for (auto i = 0; i < GetAttributesCount(); ++i) {
			auto attr = GetAttributeAt(i);
			if (attr.Name == a_name) {
	...
	auto XMLReader::getAttribute(const std::string& a_name, const std::string& def) -> std::string
	{
		for (auto i = 0; i < GetAttributesCount(); ++i) {
			auto attr = GetAttributeAt(i);
			if (attr.Name == a_name) {
	...
	auto XMLReader::getAttributeAsFloat(const std::string& a_name) -> float
	{
		return convertFloat(getAttribute(a_name));
	}

	auto XMLReader::getAttributeAsInt(const std::string& a_name) -> int
	{
		return convertInt(getAttribute(a_name));
	}

	auto XMLReader::getAttributeAsBool(const std::string& a_name) -> bool
	{
