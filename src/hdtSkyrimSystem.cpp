...
namespace hdt
{
    ...
    auto SkyrimSystem::readTransform(float timeStep) -> void
    {
        ...
        else if (m_skeleton->parent == RE::PlayerCharacter::GetSingleton()->Get3D2())
        {
            if (SkyrimPhysicsWorld::get()->m_resetPc > 0)
            {
                ...
                RE::CameraState::kFirstPerson) // isWeaponSheathed or potentially isCameraFree || cameraState
            // is first person
            {
                m_lastRootRotation = convertNi(m_skeleton->world.rotate);
            }
    ...
    {
        const auto bone = dynamic_cast<SkyrimBone*>(m_mesh->findBone(getRenamedBone(name)));
        if (bone)
        {
            return bone;
        }

        logger::warn("Bone {} used before being created, trying to create it with current default values", name);
                                                   ...
                                                   SkyrimSystem* old_system) -> RE::BSTSmartPointer<SkyrimSystem>
    {
        ...
        std::ranges::sort(m_mesh->m_bones, [](const auto& a, const auto& b)
        {
            return static_cast<SkyrimBone*>(a.get())->m_depth < static_cast<SkyrimBone*>(b.get())->
                m_depth;
        });
        ...
        if (typeStr == "hull")
        {
            ...
            while (m_reader->Inspect())
            {
                if (m_reader->GetInspected() == XMLReader::Inspected::StartTag)
                {
                    ...
                    {
                        float margin = m_reader->readFloat();
                    }
                    else
                    {
                        logger::warn("unknown element - {}", name.c_str());
    ...
    {
        const auto name = getRenamedBone(m_reader->getAttribute("name"));
        ...
        const auto cls = m_reader->getAttribute("template", "");
    ...
    auto SkyrimSystemCreator::generateMeshBody(const std::string& name, DefaultBBP::NameSet_t* names)
        ...
        -> RE::BSTSmartPointer<SkyrimBody>
    {
        ...
        for (const auto& [fst, snd] : vertexOffsetMap)
        {
            auto* g = castBSTriShape(findObject(m_model, fst.c_str()));
            ...
            {
                logger::warn("Shape {} has no skin data, skipped", fst.c_str());
                return nullptr;
            }
        }
    ...
    auto SkyrimSystemCreator::parseFrameType(const std::string& name, FrameType& frameType,
                                             btTransform& frame) const -> bool
    {
        if (name == "frameInA")
        {
