...
namespace hdt
{
    ...
    struct _uint32_to_str_t
    {
        ...
        auto operator()(uint32_t number) -> std::string
        {
            _uin32_cstr _union{};
        ...
        static auto Load(SKSE::SerializationInterface* intfc) -> void
        {
            ...
            while (intfc->GetNextRecordInfo(type, version, length))
            {
                auto record = std::find_if(
                    g_SerializerList.begin(), g_SerializerList.end(), [type, version](SerializerBase* a_srlzr)
                    {
    ...
    protected:
        static inline auto _toString(_Stream_t& _stream) -> std::string { return _stream.rdbuf()->str(); };
    };
