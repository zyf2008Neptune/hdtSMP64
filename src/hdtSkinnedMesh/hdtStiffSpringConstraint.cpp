...
namespace hdt
{
    ...
    {
        const auto distance = a->m_currentTransform.getOrigin().distance(b->m_currentTransform.getOrigin());
    ...
    auto StiffSpringConstraint::scaleConstraint() -> void
    {
        ...
        const float w0 = m_boneA->m_rig.getInvMass();
        const float w1 = m_boneB->m_rig.getInvMass();
        const auto factor = (newScaleA / m_scaleA * w0 + newScaleB / m_scaleB * w1) / (w0 + w1);
        const auto factor3 = factor * factor * factor;
    ...
    auto StiffSpringConstraint::getInfo1(btConstraintInfo1* info) -> void
    {
        auto a = m_boneA->m_rigToLocal * m_rbA.getWorldTransform();
        auto b = m_boneB->m_rigToLocal * m_rbB.getWorldTransform();
        const auto a2b = a.getOrigin() - b.getOrigin();
        const auto distance = a2b.length();

        info->m_numConstraintRows = !btFuzzyZero(distance);
        info->nub = 0;
    }

    auto StiffSpringConstraint::getInfo2(btConstraintInfo2* info) -> void
    {
        auto a = m_boneA->m_rigToLocal * m_rbA.getWorldTransform();
        auto b = m_boneB->m_rigToLocal * m_rbB.getWorldTransform();
        const auto a2b = a.getOrigin() - b.getOrigin();
        ...
        if (!currentLimit)
        {
            ...
            const auto vel = (delta - m_oldDiff) * info->fps;

            // spring force is (delta * m_stiffness) according to Hooke's Law
            btScalar force = (delta + m_oldDiff) * 0.5f * m_stiffness;
            const btScalar friction = m_damping * vel;
            force += force * friction < 0 ? btClamped(friction, -abs(force), abs(force)) : friction;
            const btScalar velFactor = info->fps / static_cast<btScalar>(info->m_numIterations);
            const auto m_targetVelocity = velFactor * force;
            const auto m_maxMotorForce = btFabs(force) / info->fps;

            const btScalar mot_fact =
                getMotorFactor(distance, m_minDistance, m_maxDistance, m_targetVelocity, info->fps * info->erp);
        ...
        {
            const btScalar k = info->fps * info->erp;
