...
namespace hdt
{
    inline auto pack256(__m128 lo, __m128 hi) -> __m256 { return _mm256_set_m128(hi, lo); }

    inline auto unpack256(__m256 ymm) -> std::tuple<__m128, __m128>
    {
    ...
    static auto gResolveSingleConstraintRowGeneric_avx256(btSolverBody& body1, btSolverBody& body2,
                                                          const btSolverConstraint& c) -> btScalar
    {
        ...
        // body1.internalGetDeltaLinearVelocity().mVec128), DOT_PRODUCT(c.m_relpos1CrossNormal.mVec128,
        // body1.internalGetDeltaAngularVelocity().mVec128)); const __m128 deltaVel2Dotn =
        // _mm_add_ps(DOT_PRODUCT(c.m_contactNormal2.mVec128, body2.internalGetDeltaLinearVelocity().mVec128),
        ...
        // deltaImpulse = _mm_blendv_ps(_mm_sub_ps(lowerLimit, c.m_appliedImpulse), _mm_blendv_ps(_mm_sub_ps(upperLimit,
        ...
        // body1.internalGetDeltaLinearVelocity().mVec128 = FMADD(_mm_mul_ps(c.m_contactNormal1.mVec128,
        // body1.internalGetInvMass().mVec128), deltaImpulse, body1.internalGetDeltaLinearVelocity().mVec128);
        // body2.internalGetDeltaLinearVelocity().mVec128 = FMADD(_mm_mul_ps(c.m_contactNormal2.mVec128,
        // body2.internalGetInvMass().mVec128), deltaImpulse, body2.internalGetDeltaLinearVelocity().mVec128);
        // body1.internalGetDeltaAngularVelocity().mVec128 = FMADD(c.m_angularComponentA.mVec128, deltaImpulse,
        // body1.internalGetDeltaAngularVelocity().mVec128); body2.internalGetDeltaAngularVelocity().mVec128 =
        // FMADD(c.m_angularComponentB.mVec128, deltaImpulse, body2.internalGetDeltaAngularVelocity().mVec128);
        return deltaImpulse.m128_f32[0] / c.m_jacDiagABInv;
    }

    // Enhanced version of gResolveSingleConstraintRowGeneric_sse2 with AVX
    static auto gResolveSingleConstraintRowLowerLimit_avx256(btSolverBody& body1, btSolverBody& body2,
                                                             const btSolverConstraint& c) -> btScalar
    {
        __m128 tmp = _mm_set_ps1(c.m_jacDiagABInv);
        __m128 deltaImpulse = _mm_set_ps1(c.m_rhs - btScalar(c.m_appliedImpulse) * c.m_cfm);
        ...
        // body1.internalGetInvMass().mVec128), deltaImpulse, body1.internalGetDeltaLinearVelocity().mVec128);
        // body1.internalGetDeltaAngularVelocity().mVec128 = FMADD(c.m_angularComponentA.mVec128, deltaImpulse,
        // body1.internalGetDeltaAngularVelocity().mVec128); body2.internalGetDeltaLinearVelocity().mVec128 =
        // FMADD(_mm_mul_ps(c.m_contactNormal2.mVec128, body2.internalGetInvMass().mVec128), deltaImpulse,
        // body2.internalGetDeltaLinearVelocity().mVec128); body2.internalGetDeltaAngularVelocity().mVec128 =
        // FMADD(c.m_angularComponentB.mVec128, deltaImpulse, body2.internalGetDeltaAngularVelocity().mVec128);
        return deltaImpulse.m128_f32[0] / c.m_jacDiagABInv;
    }
    ...
    auto NonContactSolverTask::solve() -> void
    {
        std::scoped_lock la(*m_lockOrderA);
    ...
    auto ContactSolverTask::solve() -> void
    {
        std::scoped_lock la(*m_lockOrderA);
        std::scoped_lock lb(*m_lockOrderB);
    ...
    auto ObsoleteSolverTask::solve() -> void
    {
        std::scoped_lock la(*m_lockOrderA);
        std::scoped_lock lb(*m_lockOrderB);
    ...
    auto GroupConstraintSolver::solveGroupCacheFriendlySetup(btCollisionObject** bodies, int numBodies,
                                                             ...
                                                             btIDebugDraw* debugDrawer) -> btScalar
    {
        ...
        // init solver body
        for (auto j = 0; j < numConstraints; j++)
        {
        ...
        for (auto i = 0; i < m_bodiesMt.size(); ++i)
        {
        ...
        {
            {
                ...
                {
                    m_nonContactConstraintRowPtrs.emplace_back(i);
                }

                std::ranges::sort(m_nonContactConstraintRowPtrs,
                                  [](const btSolverConstraint* a, const btSolverConstraint* b)
                                  {
                ...
                if (lastA != a || lastB != b)
                {
                    if (lastA && lastB)
                    {
                        auto task = std::static_pointer_cast<SolverTask>(std::make_shared<NonContactSolverTask>(
                            lastA, lastB, lastBegin, curr, getActiveConstraintRowSolverGeneric()));
                        m_tasks.emplace_back(task);
                        m_nonContactTasks.emplace_back(task);
                    }
                    lastA = a;
                    lastB = b;
                    lastBegin = curr;
        ...
        for (auto i = 0; i < m_tmpSolverContactConstraintPool.size(); ++i)
        {
            ...
            m_tasks.emplace_back(task);
            m_contactTasks.emplace_back(task);
        }

        std::random_device rng;
        std::mt19937 urng(rng());
        std::ranges::shuffle(m_tasks, urng);
        return ret;
    }
    ...
    auto GroupConstraintSolver::getResolveSingleConstraintRowGenericAVX() -> btSingleConstraintRowSolver
    {
        return gResolveSingleConstraintRowGeneric_avx256;
    }

    auto GroupConstraintSolver::getResolveSingleConstraintRowLowerLimitAVX() -> btSingleConstraintRowSolver
    {
    ...
    auto GroupConstraintSolver::solveSingleIteration(
        int iteration, [[maybe_unused]] btCollisionObject** bodies, [[maybe_unused]] int numBodies,
        [[maybe_unused]] btPersistentManifold** manifoldPtr, [[maybe_unused]] int numManifolds,
        ...
        const btContactSolverInfo& infoGlobal, [[maybe_unused]] btIDebugDraw* debugDrawer) -> btScalar
    {
        ...
        else
        {
            std::random_device rng;
            std::mt19937 urng(rng());
            std::ranges::shuffle(m_nonContactTasks, urng);
            std::ranges::shuffle(m_contactTasks, urng);
