...
namespace hdt
{
	...
	static bool needsCollision(const SkinnedMeshBody* shape0, const SkinnedMeshBody* shape1)
	{
	...
	void CollisionDispatcher::dispatchAllCollisionPairs(btOverlappingPairCache* pairCache,
		[[maybe_unused]] const btDispatcherInfo& dispatchInfo, [[maybe_unused]] btDispatcher* dispatcher)
	{
		...
		if (haveCuda) {
			...
			for (int i = 0; i < m_pairs.size(); ++i) {
				...
				if (pair.first->m_shape->m_tree.collapseCollideL(&pair.second->m_shape->m_tree)) {
					...
					} else if (pair.first->m_shape->asPerTriangleShape() &&
							   pair.second->m_shape->asPerTriangleShape()) {
...
#	endif
	...
	int CollisionDispatcher::getNumManifolds() const { return m_manifoldsPtr.size(); }

	btPersistentManifold* CollisionDispatcher::getManifoldByIndexInternal(int index) { return m_manifoldsPtr[index]; }
