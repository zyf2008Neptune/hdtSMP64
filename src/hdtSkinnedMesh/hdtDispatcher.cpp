...
#endif
            {
                    ...
                    if (!initialized)
                    {
                        concurrency::parallel_for_each(
                            ...
                            [deviceId](UpdateMap::value_type& o)
                            {
                                ...
                                if (o.second.first &&
                                    (!o.second.first->m_cudaObject || o.second.first->m_cudaObject->deviceId() !=
                                        deviceId))
                                {
                                    o.second.first->m_cudaObject.reset(new CudaPerVertexShape(o.second.first));
                                }
                                if (o.second.second &&
                                    (!o.second.second->m_cudaObject || o.second.second->m_cudaObject->deviceId() !=
                                        deviceId))
                                {
                ...
                if (haveCuda)
                {
                    ...
                    for (int i = 0; i < m_pairs.size(); ++i)
                    {
                        ...
                        {
                            if (!pair.first->m_shape->asPerTriangleShape() || !pair.second->m_shape->
                                asPerTriangleShape())
                            {
                                m_delayedFuncs.push_back(
                                    SkinnedMeshAlgorithm::queueCollision(pair.first, pair.second, this));
                            }
                            else if (pair.first->m_shape->asPerTriangleShape() && pair.second->m_shape->
                                asPerTriangleShape())
                            {
                                m_immediateFuncs.push_back(
                                    SkinnedMeshAlgorithm::queueCollision(pair.first, pair.second, this));
                            }
                        }
                    }
