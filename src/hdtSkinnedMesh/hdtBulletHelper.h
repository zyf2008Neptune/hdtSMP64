...
namespace hdt
{
	...
	template <int imm>
	auto pshufd(const __m128 m) -> __m128 { return _mm_castsi128_ps(_mm_shuffle_epi32(_mm_castps_si128(m), imm)); }
	...
	template <class T>
	auto abs(T rhs) -> T { return rhs < 0 ? -rhs : rhs; }

	template <>
	inline auto abs(const float rhs) -> float { return _mm_cvtss_f32(_mm_andnot_ps(_mm_set_ss(-0.f), _mm_set_ss(rhs))); }

	template <class T>
	auto min(const T& a, const T& b) restrict(cpu, amp) -> T { return a < b ? a : b; }

	template <class T>
	auto max(const T& a, const T& b) restrict(cpu, amp) -> T { return a < b ? b : a; }

	inline auto aligned(const int x, const int a) -> int { return (x + a - 1) & -a; }

	template <int a>
	auto aligned(const int x) -> int { return (x + a - 1) & -a; }
	...
	public:
		...
		auto asTransform() const -> btTransform
		{
	...
	public:
		...
		auto operator*(const btMatrix4x3T& r) -> btMatrix4x3T //should not overload operator pointer
		{
			btMatrix4x3T ret;
