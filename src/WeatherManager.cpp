...
namespace hdt
{
    ...
    auto WeatherManager::randomGenerator(int min, int max) -> int
    {
        std::uniform_int_distribution<int> dist(min, max);
        return dist(m_rng);
    }

    auto WeatherManager::randomGeneratorLowMoreProbable(int lowerMin, int lowerMax, int higherMin, int higherMax,
                                                        int probability) -> int
    {
    ...
    auto WeatherManager::clearWind(float cooldown) -> void
    {
    ...
    auto WeatherManager::runWeatherTickImpl(float delta) -> void
    {
        m_cooldown -= delta;
        if (m_cooldown > 0.f)
        {
